...
alias icontest_balfrin='ssh icontest_balfrin'
alias santis='ssh santis'
alias icontest_santis='ssh icontest_santis'
alias euler='ssh euler'
alias ela='ssh ela'
alias sc='cd $SCRATCH'
...
if [[ -z "$EXP" ]]; then
  ...
  for target in cpu_double gpu_double cpu_mixed gpu_mixed cpu_nvhpc gpu_nvhpc; do
    if [[ -d "$target" ]]; then
      ...
      cp "run/checksuite.icon-dev/check.$EXP" "$target/run/checksuite.icon-dev"
